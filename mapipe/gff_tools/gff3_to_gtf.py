...
class GTFWriteTools(object):
    ...
    def gtf_writer(gtf, s, new=False):
        if new:
            gtf_f = open(gtf, 'w', newline="\n")
        else:
            gtf_f = open(gtf, 'a', newline="\n")
        gtf_f.write(s + "\n")
        gtf_f.close()
    ...
    def read_attr(self, col9, parent_dict, id, parent, gene_name):
        ...
        for attr in col9_list:
            ...
            if attr_list[0] in defined_attrs:
                attr_dict[defined_attrs[attr]] = attr_list[1]
...
def gff3_converter(gff, id_attrt='ID', parent_attr='Parent', gene_name_attr='Name', gtf=None, **kwargs):
    ...
    for lines in gff:
        ...
        if line_list[2] not in features:
            continue
        if line_list[2] == "gene":
            gene = Gene(
            ...
            transcript.read_attr(line_list[8], genes, id_attrt, parent_attr, gene_name_attr)
            ...
            exon.read_attr(line_list[8], transcripts, id_attrt, parent_attr, gene_name_attr)
            ...
            cds.read_attr(line_list[8], transcripts, id_attrt, parent_attr, gene_name_attr)
...
def _parse_cmd_args(cmd_args):
    ...
    parser.add_argument(
        ...
        default="Name",
        required=False,
        help="Equivalent to 'Name' attribute in your file",
        action='store'
    )
    parser.add_argument(
        '-o',
